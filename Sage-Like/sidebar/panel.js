...
const panel = (function() {
	...
	function setPanelFontSizePercentFromPreferences() {
		...
		prefs.getFontSizePercent().then(percent => {
			...
			let iconSizeAddend = (percent - parseInt(prefs.DEFAULTS.fontSizePercent)) * 0.09;
			document.documentElement.style.setProperty("--addend-tree-icon-size", iconSizeAddend + "px");
		});
	}
	...
	async function showOpenInContainerPicker(url) {
		...
		text = `<div class='message-menu-subtitle'>${i18n("js_panelChooseContainerSubtitle")}</div><div class='message-menu-container'>${text}</div>`;
	...
	return {
		...
		showOpenInContainerPicker: showOpenInContainerPicker,
	};
