...
const rssTreeView = (function() {
	...
	function onDragEnterTreeItem(event) {
		...
		internalPrefs.getDropInsideFolderShowMsgCount().then((count) => {
			if(count > 0 && (m_dropInsideFolderLastShowMsgTime + 7000) < Date.now()) {	// Event is triggered repeatedly. Set 7s between messages to conserve counter.
				...
				InfoBubble.i.show(i18n("js_rssTreeBubbleMsgDropInside"), {alertive: false});
			...
			if ( !!elmCurrentlyDragged && transfer.types.includes("text/wx-sl-treeitem-id") ) {
				...
				gettingDragged.then((dragged) => {
					gettingDrop.then((drop) => {
						...
						suspendBookmarksEventHandler(() => {
							return browser.bookmarks.move(elmCurrentlyDragged.id, destination).then((moved) => {
								...
								if(transfer.getData("text/wx-sl-treeitem-id") !== elmCurrentlyDragged.id) {
									InfoBubble.i.show(i18n("js_rssTreeBubbleMsgDnDError"));
						...
						InfoBubble.i.show(i18n("js_rssTreeBubbleMsgDropTargetRemoved"));
						console.log("[Sage-Like]", "Bookmarks get error", error);
					});
				}).catch((error) => {
					InfoBubble.i.show(i18n("js_rssTreeBubbleMsgDraggedItemRemoved"));
					console.log("[Sage-Like]", "Bookmarks get error", error);
				});
	...
	function openTreeItemFeedPreview(elm, openMethod) {
		...
		switch (openMethod) {
			case URLOpenMethod.IN_TAB:					openTreeItemFeedPreviewInTab(url);						break;
	...
	////////////////////////////////////////////////////////////////////////////////////
	async function openTreeItemFeedPreviewInTab(url) {
	...
	function createNewFeed(elmLI, title, url, inFolder, details = {}) {
		...
		browser.bookmarks.get(elmLI.id).then((bookmarks) => {
			...
			InfoBubble.i.show(i18n("js_rssTreeBubbleMsgTargetRemoved"));
			console.log("[Sage-Like]", "Bookmarks get error", error);
		});
	}
		...
		prefs.getRootFeedsFolderId().then((folderId) => {
			...
			suspendBookmarksEventHandler(() => {
				return browser.bookmarks.create(newBookmark).then((created) => {
					...
					InfoBubble.i.show(i18n("js_rssTreeBubbleMsgTargetFolderRemoved"));
	...
	function deleteTreeItem(elmLI) {
		...
		messageView.open({ text: text, btnSet: messageView.ButtonSet.setYesNo, caption: cap }).then(async (result) => {
			...
			if(result.buttonCode !== messageView.ButtonCode.Yes) {
				return;
			}
			...
			suspendBookmarksEventHandler(() => {
				return funcBookmarksRemove(elmLI.id).then(() => {
					...
					InfoBubble.i.show(i18n("js_rssTreeBubbleMsgItemAlreadyRemoved"));
	...
	function toggleFeedVisitedState(elmLI) {
		...
		if(elmLI.classList.contains("error")) {
			...
			InfoBubble.i.show(text, {pointTo: getTreeItemIcon(elmLI), hideOnScroll: true});
			return;
		}
	...
	function openTreeSummary() {
		...
		const lines = [
			...
			...(!m_isFilterApplied ? [] : [
				FMT_ROW.format([i18n("js_rssTreeSummaryVisible"), nVisible]),
				FMT_ROW.format([i18n("js_rssTreeSummaryFilteredOut"), nFilteredOut])
			]),
			"</div>",
				...
				if(folderId === Global.ROOT_FEEDS_FOLDER_ID_NOT_SET) {
					...
					browser.bookmarks.search({ title: Global.DEFAULT_FEEDS_BOOKMARKS_FOLDER_NAME }).then(async (treeNodes) => {
						...
						if(!!foundNode) {
							...
							InfoBubble.i.show(i18n("js_rssTreeBubbleMsgDfaultFeedsFolder"), {alertive: false});
	...
	function openFilterWidget() {
		...
		internalPrefs.getMsgShowCountHoverFilterTextBox().then((count) => {
			if(count > 0) {
				InfoBubble.i.show(i18n("js_rssTreeBubbleMsgHoverFilter"), {pointTo: m_elmFilterWidget, alertive: false});
	...
	function showUnauthorizedInfoBubble(refElm, errorObject) {
		...
		if(syndication.isUnauthorizedError(errorObject)) {
			internalPrefs.getMsgShowCountUnauthorizedFeed().then((count) => {
				if(count > 0) {
					InfoBubble.i.show(i18n("js_rssTreeBubbleMsgSignin"), {pointTo: getTreeItemIcon(refElm), alertive: false, duration: 8000, hideOnScroll: true});
