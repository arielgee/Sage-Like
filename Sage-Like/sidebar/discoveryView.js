...
const discoveryView = (function() {
		...
		browser.tabs.query({ currentWindow: true, active: true }).then((tabs) => {
			...
			gettingPageData.then((pd) => {
				...
				if(/^(about|chrome|resource|moz-extension):/i.test(tab.url)) {
					...
					setDiscoverLoadingState(false);
					setNoFeedsMsg(i18n("js_discoveryLocalFileNotAccessable"), true);
				} else {
	...
	function setNoFeedsMsg(text, notAllowedFileSchema = false) {
		...
		if(!RequiredPermissions.i.granted) {
			elmA = createNoFeedsMsgLink(i18n("js_discoveryRequiredPermissions"), async () => panel.askForRequiredPermissions() );
		} else if(notAllowedFileSchema) {
			elmA = createNoFeedsMsgLink(i18n("js_discoveryLearnMore"), async () => panel.informAboutLocalFilesAccessPermission() );
		}
